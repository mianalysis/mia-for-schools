...
export default function SimpleIm(props: Props) {
  ...
  function getChannelComponentsDataJSON() {
    ...
    //   dataJSON.datasets[0].backgroundColor.push(
    //     rgbToHex(
    //       Math.round((3 * channel.red) / 4),
    //       Math.round((3 * channel.green) / 4),
    //       Math.round((3 * channel.blue) / 4)
    //     )
    //   );
    // }
  ...
  function getImageIntensityHistogramDataJSON() {
    // var numberOfBins = 12;
    // var labels = [];
    // for (let i = 0; i < numberOfBins; i++) labels.push(Math.floor((i * 256) / numberOfBins));
    ...
    //     Math.round((3 * channel.blue) / 4)
    //   );
    //   for (let i = 0; i < numberOfBins; i++) backgroundColour.push(hex);

    //   var datasetJSON: DatasetJSON = {
    //     label: 'Channel ' + (cIdx + 1),
    //     data: histData,
